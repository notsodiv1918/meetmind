...
export function useMOM() {
  ...
  async function generate(params: GenerateParams) {
    ...
    try {
      ...
      if (tab === 'text') {
        if (!transcript.trim()) throw new Error('Please paste a transcript in the Text tab.')
        content = [{ type: 'text', text: buildMOMPrompt(transcript, options) }]

      } else if (tab === 'images') {
        const imgs = imageFiles.filter(f => f.b64 && f.type.startsWith('image/'))
        if (!imgs.length) throw new Error('No supported image files. Add PNG, JPG, or WEBP files.')
        content = [
          ...imgs.map(img => ({
            type: 'image',
            source: { type: 'base64', media_type: img.type, data: img.b64 },
          })),
          { type: 'text', text: buildMOMPrompt('(see attached meeting screenshots)', options) },
        ]
        rawTranscript = ''
        ...
        content = [{ type: 'text', text: buildMOMPrompt(rawTranscript, options) }]
      }
