...
function detectLanguages(transcript: string): string[] {
  ...
  patterns.forEach(({ lang, re }) => { if (re.test(transcript)) langs.push(lang) })
  if (langs.length > 0 || /[a-zA-Z]/.test(transcript)) langs.unshift('English')
  return [...new Set(langs)]
}

export function parseMOMResponse(rawText: string, transcript?: string): MOMResult {
  ...
  return {
    ...
    unansweredQuestions: parseQuestions(extractSection(rawText, 'UNANSWERED QUESTIONS')),
